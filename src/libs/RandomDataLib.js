...
function createRandomNumericalData(
  n,
...
function createRandomCategoricalData(
  n,
  axis = "vertical",
  labelLength = 10,
  color = "rgba(70, 130, 180, 0.4)",
...
function createHeatmapColors(n, type, colorRange = d3.schemeCategory10) {
  if (type == "discrete") {
    const domain = d3.range(0, n);
    const colorScale = d3.scaleOrdinal().domain(domain).range(colorRange);
    const colors = domain.map((d) => colorScale(d));
    return {
 ...
 */
function createRandomStackedCategoricalData(
  nBars,
  nSeries,
  axis = "vertical",
  labelLength = 10,
...
function createSeriesColorInfo(
  nSeries,
