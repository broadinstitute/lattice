...
export class Axis extends AxisKernel {
  ...
  _validate() {
    if (
      ...
      ![axisOrientations.TOP, axisOrientations.BOTTOM].includes(
        this.orientation
      )
    ) {
    ...
    if (
      ...
      ![axisOrientations.LEFT, axisOrientations.RIGHT].includes(
        this.orientation
      )
    ) {
      throw `Invalid orientation ${this.orientation} found for ${this.axisType} axis`;
    ...
    switch (this.orientation) {
      case axisOrientations.BOTTOM:
        label
          ...
          .attr("y", plot.innerHeight + plot.padding.bottom - 4)
      ...
      case axisOrientations.LEFT:
        label
          .attr("x", -plot.padding.left / 2)
          .attr("y", -plot.padding.top / 2)
          .attr("text-anchor", "middle");
        break;
  ...
  render(svg, plot) {
    ...
    let axisFn = this._getAxisFn(this.orientation)
      .scale(this._scale)
      .tickSizeOuter(0);
    ...
    if (this.textAnchor) {
      axis.selectAll("text").style("text-anchor", this.textAnchor);
    }
    if (this.hideLabels) {
