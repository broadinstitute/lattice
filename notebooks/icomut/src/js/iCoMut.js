...
export default class CoMutationPlot {
    constructor(rootId, dataFiles={config: configFile, data: testFile}, width, height) {
        this.rendered = false;
        this.rootId = rootId;
        ...
        this.dimensions = {
            width: width || window.innerWidth * .9,
    ...
    _createPanels() {
        this.config.panels.forEach(p => {
            const panel = new Panel(p.id, p.label, p.searchKeyword, p.row, p.plots, p.colors, p.panelOptions, this);
     ...
     */
    _initSort() {
        if (this.config.initialSort === undefined || !this.config.initialSort.length) {
    ...
    _initLattice() {
        ...
        const latticeConfig = { width: width, height: height, grid: this.config.grid };
        ...
        this.sampleBasedPlots = this.lattice.plots.filter(p => p.column == sampleBasedColumn);
     ...
     */
    _sortSamples() {
        const samples = this.allSamples.filter(s => !this.excludedSamples.includes(s));
        const compareFn = (a, b) => {
            /**
            ...
            const getSampleVal = (sId, keyword) => {
                ...
                if (cols.length > 1) {
                    val = cols.reduce((x, y) => data[y] === undefined ? x : x + data[y], 0);
                } else {
...
export function init(rootId, dataFiles={config: configFile, data: testFile}, width, height) {
