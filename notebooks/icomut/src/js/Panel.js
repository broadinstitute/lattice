...
export class Panel {
  ...
  _parsePanelData() {
    switch (this.mainPlotType) {
      case "categoricalheatmap": {
        const mainPlotInfo = dataParser.parseCategoricalHeatmapData(
          this.comutObj.data,
          this
        );
        this._full[this.sampleBasedCol] = mainPlotInfo;
        this.yColumns =
          mainPlotInfo.columns.length > 1
            ? mainPlotInfo.columns.map((d) => d.replace(this.searchKeyword, ""))
            : mainPlotInfo.columns;
        ...
        this.panelCountData = dataParser.getPanelCountData(
          this._full[this.sampleBasedCol].plotData
        );
        const countPlotInfo = dataParser.parseStackedCountData(
          this.panelCountData,
          this
        );
        ...
        const heatmapTtipFormatter = (d) => {
          const tooltipData = [
            `${d.x}`,
            `${d.y}`,
            `${this.clusterKey ? this.clusterKey[d.c] : d.c}`,
          ];
          return tooltipData.join("<br/>");
        };
        const countTtipFormatter = (d) => {
          const tooltipData = [
            `${this.clusterKey ? this.clusterKey[d.series] : d.series}`,
            `Count: ${d.data[d.series]}`,
          ];
          return tooltipData.join("<br/>");
        };
      ...
      case "stackedcolumnplot": {
        const mainPlotInfo = dataParser.parseStackedData(
          this.comutObj.data,
          this
        );
        this._full[this.sampleBasedCol] = mainPlotInfo;
      ...
      case "stripplot": {
        ...
        this._full[this.sampleBasedCol] = mainPlotInfo;
        this.yColumns = mainPlotInfo.columns.map((d) =>
          d.replace(this.searchKeyword, "")
        );
        this.sortedYColumns = this.yColumns.map((d) => d); // mapping to create new copy
  ...
  _setDataToDisplay() {
    ...
    const sampleExcludedData = this._full[mainCol].plotData.filter(
      (d) => !excludedSamples.includes(d.x)
    );
    ...
    switch (this.mainPlotType) {
      ...
      case "categoricalheatmap": {
        ...
        if (sort == "count") {
          const countData = dataParser.getPanelCountData(
            this._full[mainCol].plotData
          );
          const countList = Object.keys(countData).map((d) => ({
            total: countData[d].total,
            y: d,
          }));
          countList.sort((a, b) => {
          ...
          this.sortedYColumns = countList
            .filter((_, i) => lim === undefined || i < lim)
            .map((d) => d.y);
        } else if (sort == "qval") {
          const dataTransform = dataParser.dataTransformer.Q;
          const qLim = lim === undefined ? undefined : dataTransform(lim);
          const filteredQData = qValData.filter(
            (d) => qLim === undefined || d.c >= qLim
          );
        ...
        this._displayedData[mainCol] = sampleExcludedData.filter((d) =>
          this.sortedYColumns.includes(d.y)
        );
        this._displayedData[mainCol - 1] = countPlotInfo.plotData.filter((d) =>
          this.sortedYColumns.includes(d.y)
        );
        this._displayedData[mainCol + 1] = qValData.filter((d) =>
          this.sortedYColumns.includes(d.y)
        );
        break;
      }
    ...
    const configs = Object.keys(this._full).map((col) => {
      ...
      switch (pConfig.plotType) {
        case "categoricalheatmap":
          ...
          config.axis.c = {
            domain: [...Array(this.colors.length).keys()],
            range: this.colors,
          };
          break;
        case "heatmap":
        ...
        case "stackedcolumnplot":
          config = {
            ...
            tooltip: {
              formatter: (d) => {
                const tooltipDataList = Object.keys(d.data).map(
                  (x) => `${x}: ${d.data[x]}`
                );
        ...
        case "stripplot":
          ...
          config.axis.c = {
            domain: pDataConfig.domain,
            range: pDataConfig.range,
          };
          break;
        default:
      }
